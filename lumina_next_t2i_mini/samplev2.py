    ...
    with torch.autocast("cuda", dtype):
        for res in resolution:
            for idx, caption in tqdm(enumerate(captions)):
                ...
                # Latent
                z = torch.randn([1, 4, 2, latent_w, latent_h], device="cuda").to(dtype)
                z = z.repeat(n * 2, 1, 1, 1, 1)

                # Latent Motion Noise
                zmf = torch.randn([1, 4, 2, latent_w, latent_h], device="cuda").to(dtype)
                ...
                samples = samples.permute(0, 2, 1, 3, 4).squeeze(dim=0)
